...
plot(Y ~ X, data=trainset, pch='X', xlab='', ylab='',xaxt='n', ann=FALSE, yaxt='n')
plot(y~x, pch=16, xlab='', ylab='',xaxt='n', ann=FALSE, yaxt='n')
points(Y ~ X, data=trainset, pch='X', col='red2')
plot( function(x) x^(1/3), max(trainset$X), col='red', lwd=3, xlab='', ylab='',xaxt='n', ann=FALSE, yaxt='n')  
points(Y ~ X, data=trainset, pch='X')

# True fit
truefit <- lm(Y ~ I(X^(1/3)), data=trainset)
Y_train <- predict(truefit, newdata=trainset)
lines(trainset$X, Y_train)
# Poly 1
fit
# Poly 10
fit <- lm(Y ~ poly(X, 3, raw=TRUE), data=trainset)
lines(x, predict(fit), col="blue", lwd=3) 

# Poly 100
fit2 <- lm(Y ~ poly(X, 100, raw=TRUE), data=trainset)
lines(x, predict(fit2), col='green3', lwd=3)
